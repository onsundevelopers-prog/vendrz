  ...
  return (
    <motion.div
      ...
      className={`spotlight-card relative flex flex-col rounded-2xl p-7 shadow-glow ${
        tier.featured ? "glass-border-emerald glass-glow border-beam" : "glass-border glass-glow"
      }`}
    >
        ...
        <ul className="mt-7 flex-1 space-y-3">
          {tier.features.map((f) => (
            <li key={f} className="flex items-start gap-2.5 text-[14px]">
              <span aria-hidden="true" className="mt-[7px] size-1.5 shrink-0 rounded-full bg-white/40" />
