...
/*                                                                     */
/*  `server` is the auth snapshot resolved by the server layout, so    */
/*  the shell renders instantly for signed-in users and enrichment     */
/*  (name/email) lands when Clerk's client session boots.              */
/* ------------------------------------------------------------------ */
...
function WorkspaceShell({
  ...
  const [ready, setReady] = useState(() => auth.isLoaded && !!auth.id);
